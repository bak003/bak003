...
download_script_file(){
    ...
    [ -n "$path" ] && \
        wget -q --no-check-certificate https://raw.githubusercontent.com/Lozy/danted/master/${path} -O ${filename}
...
download_script_file "script/sockd" "${BIN_SCRIPT}" "execute"
...
if [ -n "$(ls -l /sbin/init | grep systemd)" ];then
    download_script_file "script/sockd.service" "/lib/systemd/system/sockd.service"
